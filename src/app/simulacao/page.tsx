  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <form onSubmit={handleSimulate} className="space-y-5">
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-indigo-500 focus:outline-none"
              >
                {categorias.map((c) => (
            ...
            <div>
              <div className="flex justify-between text-xs font-semibold uppercase text-slate-400 mb-1">
                ...
                <span className="text-indigo-400 font-bold">{reducaoPercentual}%</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:bg-indigo-500 active:scale-95 flex items-center justify-center gap-2"
            >
        ...
        <div className="lg:col-span-7 flex flex-col justify-between rounded-2xl border border-white/10 bg-slate-900/60 p-6 backdrop-blur-sm">
          <div>
            <h2 className="text-base font-semibold text-white mb-2">Resultado da Análise</h2>
            <p className="text-xs text-slate-400 mb-6">
              Categoria selecionada: <span className="text-indigo-300 font-semibold">{selectedCategoria?.nome || '—'}</span>
            </p>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-xl border border-indigo-500/30 bg-indigo-950/20 p-4">
                    ...
                    <p className="text-3xl font-extrabold text-white mt-1">{result.q3}</p>
                    <p className="text-[11px] text-slate-400 mt-1">Meta de Produtividade Elevada</p>
                  </div>
                  ...
                  <div className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
                    ...
                    <p className="text-3xl font-extrabold text-slate-200 mt-1">{result.fallback}</p>
                    <p className="text-[11px] text-slate-400 mt-1">Gatilho de Segurança de Redução</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-current/20 flex justify-between items-center text-xs font-semibold">
                    <span>Valor de Lotação Recomendado:</span>
                    <span className="text-lg font-black">{result.value}</span>
                  </div>
                </div>
              </div>
            ) : (

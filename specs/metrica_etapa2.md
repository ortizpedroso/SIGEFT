...
- Página oculta `/documentacao` (autenticada). O banner de topo (`.doc-hero`) trava contraste: navy + texto branco no escuro; cartão branco + título `#0b1736` e lead `#1e293b` no claro. O lead do topo usa `text-align: justify` na largura total do banner.
